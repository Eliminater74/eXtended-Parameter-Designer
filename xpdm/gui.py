...
class Application:
    def __init__ (self):
        ...
        self.LOCALDATADIR = os.path.join (glib.get_user_data_dir (), "xpd").decode ('utf-8')
        if not os.access (self.LOCALDATADIR, os.F_OK):
            os.makedirs (self.LOCALDATADIR, 0700)

        self.CONFIGDIR = os.path.join (glib.get_user_config_dir (), "xpd").decode ('utf-8')
...
""") % { "prof" : prof.Description, "ctrl" : prof.GetModel (), "port" : serport })
    ...
    def on_ButtonCreate_clicked (self, but):
        self.CreateProfileName.set_text (_("New profile").decode ("utf-8"))
    ...
    def on_ButtonDownload_clicked (self, but):
        ...
        self.DownloadProfileName.set_text (_("New profile").decode ("utf-8"))
...
""") % { "prof" : prof.Description, "family" : prof.Family, "port" : serport,
    ...
    def on_ControllerFamily_changed (self, cb):
        ...
        prof = fam.CreateProfile (self.ActiveProfile.FileName.decode (FNENC))
