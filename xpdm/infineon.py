...
class Profile:
    ...
    def __init__ (self, Family, FileName, ControllerModelDesc, ControllerParameters):
        ...
        for parm, desc in self.ControllerParameters.items ():
            if desc.has_key ("Default"):
                setattr (self, parm, desc ["Default"])


    def __setattr__ (self, attr, val):
        if self.__dict__.has_key ("ControllerParameters"):
            if self.ControllerParameters.has_key (attr):
                parmdesc = self.ControllerParameters [attr]
                if parmdesc.has_key ("BitField"):
                    if self.__dict__.has_key (parmdesc ["BitField"]):
    ...
    def __getattr__ (self, attr):
        ...
        # because __getattr__ is called as last resort, e.g. __dict__ has been already checked
    ...
    def LoadRaw (self, data, name_wildcard):
        ...
        for x in self.ParamRawOrder:
            if (type (x) == str) and (x != "ControllerModel"):
                p = data [idx]
                if self.ControllerParameters [x].has_key ("FromRaw"):
